...
class EuclideanVector {
 public:
  /* ctors */
  explicit EuclideanVector(
      int dimensions = 1);  // default dimension 1 ctor (i dimensions, all magnitudes 0)
  ...
  EuclideanVector(std::vector<double>::const_iterator begin,
                  std::vector<double>::const_iterator end);  // iterate over vector
  EuclideanVector(const EuclideanVector& original);          // copy construct
  EuclideanVector(EuclideanVector&& recyclee) noexcept;  // move construct ((1) noexcept as a failed
                                                         // move does not guarantee valid state)
  ...
  double& operator[](int i);       // return reference to value at index i
  double operator[](int i) const;  // return value at index i
  ...
  friend bool operator==(const EuclideanVector& a, const EuclideanVector& b) {
    ...
    if (a.dimensions_ != b.dimensions_)
      return false;
    return std::equal(a.magnitudes_.get(), a.magnitudes_.get() + a.dimensions_,
                      b.magnitudes_.get());
  }
  ...
  friend EuclideanVector operator+(const EuclideanVector& a, const EuclideanVector& b) {
    if (a.dimensions_ != b.dimensions_) {
      std::stringstream ss;
      ss << "Dimensions of LHS(" << a.dimensions_ << ") and RHS(" << b.dimensions_
         << ") do not match";
      throw EuclideanVectorError(ss.str());
    }
    EuclideanVector new_ev{a.dimensions_};
    std::transform(a.magnitudes_.get(), a.magnitudes_.get() + a.dimensions_, b.magnitudes_.get(),
                   new_ev.magnitudes_.get(), std::plus<double>());
    return new_ev;
  }
  ...
  friend EuclideanVector operator-(const EuclideanVector& a, const EuclideanVector& b) {
    ...
    EuclideanVector new_ev{a.dimensions_};
    std::transform(a.magnitudes_.get(), a.magnitudes_.get() + a.dimensions_, b.magnitudes_.get(),
                   new_ev.magnitudes_.get(), std::minus<double>());
    return new_ev;
  }
  ...
  friend double operator*(const EuclideanVector& a, const EuclideanVector& b) {
    ...
    std::transform(a.magnitudes_.get(), a.magnitudes_.get() + a.dimensions_, b.magnitudes_.get(),
                   std::back_inserter(sums), std::multiplies<double>());
  ...
  friend EuclideanVector operator*(const EuclideanVector& ev, double scaler) {
    ...
    std::transform(ev.magnitudes_.get(), ev.magnitudes_.get() + ev.dimensions_,
                   scaled_ev.magnitudes_.get(), [scaler](double mag) { return mag *= scaler; });
    return scaled_ev;
  }
  ...
  friend std::ostream& operator<<(std::ostream& os, const EuclideanVector& ev) {
    os << "[";
    std::copy(ev.magnitudes_.get(), ev.magnitudes_.get() + ev.dimensions_,
