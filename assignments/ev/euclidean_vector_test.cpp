/*
  ...
  for this assignment (and because i'm very new to lots of these c++ concepts e.g. operator
  overloading, different constructor types etc.) has been to try and test as many different cases as
  I can. I've tried to only include tests that are doing something fundamentally different to all
  other tests (i.e. testing on different objects/values) but i've allowed for possible meaningless
  tests in an attempt to try get all x cases that I need
  TLDR: lots of tests because i'm new to these things and am not sure what needs explcitly testing
  or not (so just tested all variations I could think of, even if they are superfluous)

  - my approach to writing a specific test pretty much involved asking: can I or does it make sense
  to/is it useful to test this function/method/operator etc. for;
    - a 0-dimensional vector
    - 1d vector with 0 magnitude
    - 1d vector non-0 magnitude
    - 2d vector 0 magnitudes
    - 2d vector non-zero and same/uniform magnitudes
    - 2d+ vector with differing magnitudes
    - const vs non-const vector
    - can it throw?
    - should it change the involved object(s)?
    note: I also threw in a couple negative magnitudes here and there to test they work, but didn't
  do it explicity for each method/operator like with the above scenarios note: there were also less
  common scenarios that i tested specific to each method/operator

  - I also feel it's worth mentioning that my dtor testing was just to run it on a bunch of
  different EV's and if the program doesn't throw/crash etc. then I considered them correct
  - furthermore, when it came to testing objects not being changed as a side-effect when they
  shouldn't be, if the function was const i didn't check as that is a baked in guarantee of c++ e.g.
  for the  copy assignment operator= overload, I do not explicitly test that the argument EV "const
  EuclideanVector& original" (i.e. the one being copied from) is unchanged as the const keyword
  should enforce this to be the case.
  ...
  - extent and thoroughness: I'm very happy with my testing, there's still probably edge cases I
  haven't thought of because that's generally just how it is. If anything, my own self-critique
...
SCENARIO("the subscript operator [] should have getter/setter semantics for non-const ev's "
  ...
  GIVEN("a 1d non-const ev with unit magnitude") {
    ...
    WHEN("we subscript access this ev and save the return in a reference") {
      double& dr = ev[0];
      THEN("the reference should be of unit value") { REQUIRE(dr == 1.0); }
    ...
    AND_WHEN("we directly set the subcript access equal to a valid magnitude value") {
      ev[0] = 9.0;
      THEN("the value at the accessed index should have been updated") { REQUIRE(ev[0] == 9.0); }
    }
  }
  ...
  GIVEN("a const ev") {
    ...
    AND_WHEN("we store the subscript access return in a const variable") {
      ...
      THEN("the const variable should have the value at that index") { REQUIRE(c_mag_cpy == 3.0); }
    }
  }
}
...
SCENARIO("operator std::vector<double>") {
  GIVEN("a zero dimensional ev") {
    ...
    WHEN("we cast this ev to a vector type") {
      ...
      THEN("this vec should contain no elements") { REQUIRE(vec.size() == 0); }
    }
  }
  GIVEN("a 1d ev") {
    EuclideanVector ev{1, 2.0};
...
SCENARIO("operator std::list<double>") {
  GIVEN("a zero dimensional ev") {
    ...
    WHEN("we cast this ev to a list type") {
      ...
      THEN("this list should contain no elements") { REQUIRE(list.size() == 0); }
    }
  }
  GIVEN("a 1d ev") {
    EuclideanVector ev{1, 2.0};
...
SCENARIO("friend operator ev / scaler") {
  GIVEN("any EV") {
    ...
    WHEN("we try to divide by 0") { REQUIRE_THROWS_WITH(a / 0, "Invalid vector division by 0"); }
  }
  GIVEN("a vector with 0 mags") {
    EuclideanVector ev{2, 0.0};
...
SCENARIO("method at()") {
  ...
  GIVEN("a const ev") {
    ...
    WHEN("we get from it") {
      double d = a.at(0);
      THEN("we should get the correct value") { REQUIRE(d == 2.0); }
    }
...
SCENARIO("method GetEuclideanNorm()") {
  ...
  GIVEN("a vec with 0-mags") {
    ...
    WHEN("we get it's normal") {
      ...
      THEN("it should be 0") { REQUIRE(norm == 0); }
    }
  }
  GIVEN("a vec with 1.0 mags") {
    ...
    WHEN("we get it's normal") {
      double norm = ev.GetEuclideanNorm();
      THEN("it should be the square root of the sum of mag squares") { REQUIRE(norm == 2); }
    }
  }
  GIVEN("a vec with non-zero positive mags") {
    ...
    WHEN("we get it's normal") {
      double norm = ev.GetEuclideanNorm();
      THEN("it should be the square root of the sum of mag squares") { REQUIRE(norm == 6); }
    }
  }
