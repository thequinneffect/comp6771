...
bool gdwg::Graph<N, E>::InsertEdge(const N& src, const N& dst, const E& w) {
  ...
  // std::cout << "edge [" << src << ", " << dst << ", " << w << "] was not found in find(s,d,w) and
  ...
  //     std::cout << "inserted (or prevented by) : " << (*new_edge_it)->src_->value_ <<
  //     (*new_edge_it)->dst_->value_ << (*new_edge_it)->value_ << "\n";
  // }
  // std::cout << "inserted edge was: " << src_it->get()  << " " << dst_it->get() << " " <<
  // (*new_edge_it)->value_ << "\n";
  ...
  // std::cout << "updating src node success: " << out_succ << ", updating dst node success: " <<
  // in_succ << "\n";
...
gdwg::Graph<N, E>::find(const N& src, const N& dst, const E& w) const {
  for (auto it = cbegin(); it != cend(); ++it) {
    if (src == std::get<0>(*it) && dst == std::get<1>(*it) && w == std::get<2>(*it)) {
      // std::cout << "node found was: " << std::get<0>(*it) << ", " << std::get<1>(*it) << ", " <<
      // std::get<2>(*it) << "\n";
      return it;
    }
  }
